...
function pLegendreAt(p :: Integer, x :: AbstractFloat)
    Lp = 0.0
    for k ∈ 0:p
        Lp += binomial(p, k) * binomial(p + k, k) * (0.5 * (x - 1 )) ^ (k+ 1e-30)
    end
    return Lp
end
...
function dLegendreAt(p :: Integer, x :: AbstractFloat)
    dLp = 0.0
    for k ∈ 0:p
        dLp += binomial(p, k) * binomial(p + k, k) * 0.5 * k * (0.5 * (x - 1 + 1e-30)) ^ (k - 1)
    end
    return dLp
end
...
function vcjh(p :: Integer, x :: AbstractFloat; C :: Union{Integer, AbstractFloat} = 1,
    ...
    # Alterado por mim para incluir os polinômios de Radau que Gustavo Implementou
    hL = 0.5 * ((-1.0) ^ p) * (Lp - (ηp * Lpm + Lpp*(0.5*sign(C)+0.5)+Lpm*(-0.5*sign(C)+0.5)) / (1 + ηp))
    hR = 0.5 * (Lp + (ηp * Lpm + Lpp*(0.5*sign(C)+0.5)+Lpm*(-0.5*sign(C)+0.5)) / (1 + ηp))

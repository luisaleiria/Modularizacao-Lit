...
- w[3]: Pressure.
"""
@fastmath function flux(w, PDE :: Integer, γ :: AbstractFloat = 1.4)
    # Linear advection
    return w
    #=if PDE == 1
        return w
...
@fastmath function montanteflux(u :: AbstractFloat, fL :: AbstractFloat, fR :: AbstractFloat)
    return 0.5*(fL*(1+sign(u))+fR*(1-sign(u)))
end

"""
    LLF(fL :: AbstractFloat, fR :: AbstractFloat)

Riemann solver LLF.
"""
@fastmath function LLF(uL :: AbstractFloat, uR :: AbstractFloat, fL :: AbstractFloat, fR :: AbstractFloat, a1, a2, a3)
    a = maximum([a1,a2,a3])
    # flux splitting
...
"""
=#
@fastmath function primitive(α, ρ, u, PDE)
    if PDE == 1 
        return u
    elseif PDE == 2
        return α*u #α*u*ρ
